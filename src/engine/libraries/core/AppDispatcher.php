...
class AppDispatcher
{
    ...
    private function recursiveRequestUriCheck(int &$intControllerMethodIndex, array $lstControllerBindings, $objRequestUri) : void
    {
        ...
        {
            if ($controllerBindingKey === $objRequestUri[$intControllerMethodIndex] || $currControllerBinding === $objRequestUri[$intControllerMethodIndex])
            {
    ...
    public function runApiControllerRequest(array $objRequestUri) : bool
    {
        ...
        if (!empty($objRequestUri[3])) {
            ...
            $strNoneRootProcessControllerPath = "Http\\". ucwords($strProcessModuleName) . "\Controllers\Api\\" . ucwords($strApiVersion) . "\\" . buildControllerClassFromUri($strNonRootApiControllerFileName) . "Controller";
