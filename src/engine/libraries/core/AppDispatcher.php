...
class AppDispatcher
{
    ...
    private function mainOrSubControllerBinding($objAppEntities, $objRequestUri, $intUriIndex) : ExcellTransaction
    {
        ...
        for($currUriIndex = 0; $currUriIndex < $intTotalRequestUriCount; $currUriIndex++)
        {
            if (!empty($objRequestUri[$intUriIndex + $currUriIndex]) && !empty($objAppEntities[$objRequestUri[$intUriIndex + $currUriIndex]]))
            {
                ...
                $strControllerTestAttempt = AppEntities . $strRealModule . "/" . $objAppEntities[$strRealModule]["Main"]["Folders"]["Controllers"] . "/" . buildControllerClassFromUri($strBoundControllerRequest) . "Controller" . XT;
    ...
    private function checkForPortalControllerBinding($objEntityIndexBinding, $objRequestUri, $objAppEntities, $intUriIndex) : ExcellTransaction
    {
        ...
        foreach($lstControllerBindings as $currBindingKey => $currBinding)
        {
                ...
                )
                {
                    foreach($currBinding as $currSubBindng)
                    {
                        if ( $currSubBindng === $objRequestUri[$intUriIndex + 1] )
                        {
                            ...
                            $strControllerTestAttempt = AppEntities . $strRealModule . "/" . $objAppEntities[$strBoundModule]["Main"]["Folders"]["Controllers"] . "/" . $strBoundControllerRequest . "Controller" . XT;
            ...
            elseif ($currBinding === $objRequestUri[$intUriIndex] && !empty($objAppEntities[$objRequestUri[$intUriIndex + 1]]) && in_array($currBinding, $this->app->lstPortalBindings, true))
            {
                ...
                $strControllerTestAttempt = AppEntities . $strRealModule . "/" . $objAppEntities[$strBoundModule]["Main"]["Folders"]["Controllers"] . "/" . ucwords($strBoundControllerRequest) . "Controller" . XT;
    ...
    private function checkForDefaultControllerRequest($strControllerRequest, $strControllerMethodRequest, $objRequestUri, $objAppEntities, $intUriIndex) : ExcellTransaction
    {
        ...
        if(!empty($objRequestUri[$intUriIndex]) && !empty($objAppEntities[$objRequestUri[$intUriIndex]]))
        {
            ...
            $strControllerTestAttempt = AppEntities . $objAppEntities[$objRequestUri[$intUriIndex]]["ModulePath"] . "/" . $objAppEntities[$objRequestUri[$intUriIndex]]["Main"]["Folders"]["Controllers"] . "/" . $strControllerRequest . "Controller" . XT;
            ...
            $strControllerIndexTestAttempt = AppEntities . $objAppEntities[$objRequestUri[$intUriIndex]]["ModulePath"] . "/" . $objAppEntities[$objRequestUri[$intUriIndex]]["Main"]["Folders"]["Controllers"] . "/IndexController" . XT;
    ...
    private function executeControllerMethod($objController, $strControllerMethod, ExcellActiveController $objModuleData, ExcellHttpModel $objHttpRequest) : bool
    {
        try
        {
    ...
    public function runApiControllerRequest(array $objRequestUri) : bool
    {
        ...
        $strProcessControllerPath = "Entities\\". ucwords($strProcessModuleName) . "\Controllers\Api\\" . ucwords($strApiVersion) . "\ApiController";
        ...
        if (!empty($objRequestUri[4]))
        {
            ...
            $strProcessControllerPath = "Entities\\". ucwords($strProcessModuleName) . "\Controllers\Api\\" . ucwords($strApiVersion) . "\\" . ucwords($strApiControllerFileName) . "Controller";
        }
