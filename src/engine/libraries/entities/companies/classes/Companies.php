...
class Companies extends AppEntity
{
    ...
    public function getByUuid($uuid) : ExcellTransaction
    {
        $objWhereClause = "
            ...
            (SELECT CONCAT(user.first_name, ' ', user.last_name) FROM `ezdigital_v2_main`.`user` WHERE user.user_id = cp.owner_id LIMIT 1) AS owner,
            (SELECT COUNT(*) FROM `ezdigital_v2_main`.`card` cd WHERE cd.company_id = cp.company_id) AS cards,
            (SELECT value FROM `ezdigital_v2_financial`.`user_payment_property` upp WHERE cp.company_id = upp.company_id AND cp.owner_id = upp.user_id AND upp.type_id = 2 AND upp.state = 'all' LIMIT 1) AS platform_stripe_id
            FROM `ezdigital_v2_main`.`company` cp ";
