...
class CardPageRels extends AppEntity
{
    ...
    public function getCardPageAndRelWithWidgetByRelId($intRelId) : ExcellTransaction
    {
        $strCardPageRelQuery = "SELECT 
                ...
                FROM ezdigital_v2_main.card_tab_rel ctr 
                LEFT JOIN ezdigital_v2_main.card_tab ct ON ctr.card_tab_id = ct.card_tab_id 
                LEFT JOIN ezdigital_v2_main.card cd ON ctr.card_id = cd.card_id 
                LEFT JOIN ezdigital_v2_main.product pd ON cd.product_id = pd.product_id 
                LEFT JOIN ezdigital_v2_main.product_enduser pdu ON pd.product_enduser_id = pdu.product_enduser_id 
