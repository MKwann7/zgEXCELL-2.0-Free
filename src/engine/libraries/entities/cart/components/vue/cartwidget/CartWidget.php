...
class CartWidget extends VueComponent
{
    ...
    protected function renderComponentMethods() : string
    {
        ...
        return '
            ...
            {
                if (typeof this.user.Roles === "undefined" || this.user.Roles.length === 0) {
            ...
            addPackageToCart: function(package)
            {
                ...
                if (this.inModal === false) {
                    ...
                    appCart.addPackageToCart(package.package_id)
            ...
            {
                if (!this.productCartContainsCard() && ! this.packageContainsCard(package) && this.cartIsCardProcess()) 
                { 
                    return;
                }
            ...
            openAssignUser: function(useFloat)
            {
                ...
                // If product contains Item without parent, fail.
                if (!this.productCartContainsCard() && this.cartIsCardProcess())
                {
                    return;
                }
            ...
            addPackageToCartById: function(id)
            {
                const self = this;
                ajax.PostExternal(this.rootUrl + "/cart/get-package-by-id-for-cart?id=" + id, {}, true, function(result) 
                {
            ...
            loginCustomer: function()
            {
                ...
                this.$parent.$parent.loginCardUser(
                    ...
                    function(result) 
                    {
                        ...
                        self.$parent.$parent.registerAuth(result.data);  
    
                        self.cartUser = {
                            id: result.data.user,
                            first_name: result.data.userInfo.first_name,
                            last_name: result.data.userInfo.last_name,
                            user_email: result.data.userInfo.user_email,
                            user_phone: result.data.userInfo.user_phone
                        };
        ...
        <div class="appCartWrapper products" v-bind:class="{\'private-cart\': cartType === false, \'public-cart\': cartType === true}">
            <v-style type="text/css">'.$this->renderCss(). '
                ...
                .appCartWrapper .view-cart-button.disabled .small-cart-icon {
                    background: url('.$app->objCustomPlatform->getFullPortalDomainName().'/_ez/images/financials/cart-icon-white.png) center center / auto 100% no-repeat;
                }
                ...
                .appCartWrapper .empty-cart-text .cart-icon-large {
                    background: url('.$app->objCustomPlatform->getFullPortalDomainName().'/_ez/images/financials/cart-icon-white.svg) no-repeat center center / auto 100%;
                    display: inline-block;
                    width: 80px;
                    height: 75px;
                    vertical-align: middle;
            ...
            <div v-if="displayComponent === \'selectPackagesByClass\'" class="cart-display-box entityDashboard">
                    ...
                    <li v-for="currPackage in packageList" v-bind:class="\'width cartPackageItem itemCount\' + getMaxWidthColumnCount()">
                        <div v-bind:class="\'cartPackageItemTile card-tile-\' + calculateProductCartDisplayCount()">
                            ...
                            <div class="card-tile-inner">
                                <h4 class="card-tile-title">{{ currPackage.name }} <span v-if="currPackage.enduser_id > 1"> ({{ renderEndUserLabel(currPackage) }} ONLY)</span></h4>
                ...
                <div v-if="inModal" style="text-align:center;padding-top:5px;">
                    <div v-if="!productCartContainsCard && this.cartIsCardProcess()" style="color:#cc0000;margin-bottom:15px;">This checkout requires the purchase of a new card.</div>
                    <div v-bind:class="{disabled: !productCartContainsCard}" class="view-cart-button pointer" v-on:click="openCart">
                ...
                <div v-if="cartItemsList.length > 0" class="cart-total-price">
                    ...
                    <div class="product-price-submit-wrapper">
                        <div class="product-price-info3" style="margin-top: 15px;">
                            ...
                            <button v-bind:class="{external: cartType === true, internal: cartType === false, disabled: !productCartContainsCard }" v-on:click="openAssignUser(true)" class="buttonID23542445 btn btn-warning w-100"">Go To Checkout</button>
                        </div>
                    </div>
                </div>
            </div>
