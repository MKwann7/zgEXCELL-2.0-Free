...
class ModuleApps extends AppEntity
{
    ...
    protected function getLatestModuleWidgets($whereClause) : ExcellTransaction
    {
        $strModuleQuery = "SELECT mw1.*, m1.name AS module_name ".
            "FROM ezdigital_v2_modules.module_apps mw1 " .
            "JOIN (SELECT COUNT(mw3x.version) AS versionCount, mw3x.app_uuid FROM ezdigital_v2_modules.module_apps mw3x GROUP BY mw3x.app_uuid) mw3 " .
            "JOIN ezdigital_v2_modules.module_apps mw2 ON ((mw1.app_uuid = mw2.app_uuid AND mw1.version > mw2.version) OR (mw1.app_uuid = mw2.app_uuid AND mw3.app_uuid = mw2.app_uuid AND mw3.versionCount = 1)) " .
            "JOIN (SELECT m1.* FROM ezdigital_v2_modules.modules m1 JOIN ezdigital_v2_modules.modules m2 ON m1.module_uuid = m2.module_uuid AND m1.version > m2.version ORDER BY m1.name ASC) m1 " .
            "WHERE " . $whereClause;
