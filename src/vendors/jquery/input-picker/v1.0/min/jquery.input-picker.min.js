...
(function(factory) {
    ...
    function _filterData(input, filter) {
        ...
        if (filter == true || (!_set(input, 'filterOpen') || !input_value_low || _set(input, 'url') || !_isArray(data) ) ){
            return data;
        }
     ...
     * @private
     */
    function _dataRender(input, filter) {
        if (_typeIsMultiple(input)){
            _dataRenderMultiple(input,filter);
        }
        else if (_typeIsTag(input)){
            _dataRenderTag(input,filter);
        }
        else{
            _dataRenderDefault(input,filter);
        }
    }
    ...
    function _renderTableHeader(input) {
        ...
        if(_set(input, 'headShow')){
            ...
            for(var i = 0; i < fields.length; i++){
                ...
                output += '<div class="divCell">' + text + '</div>';
            }
            output += '</div></div>';
        }
        return output;
     ...
     * Draw multiple values
     */
    function _dataRenderMultiple(input,filter){

        var inputpicker_div = _getInputpickerDiv(input);
        var wrapped_list = _getWrappedList(input);
        var uuid = _uuid(input);
        // var data = _set(input, 'data');
        var data = _filterData(input,filter);
        var fields = _set(input, 'fields');
        var fieldValue = _set(input, 'fieldValue');
        ...
        // Draw table
        output += "<div class=\"divTable inputPickerWrapper\">" ;
        ...
        if(data.length){
            ...
            output += "<div class='divTBody' style='column-span: all;'><div class='divRow'><div class='divCell' style='column-span: all;' class=\"inputpicker-no-result\">No results.</div></div></div>";
        }
     ...
     * @private
     */
    function _dataRenderDefault(input,filter) {
        var inputpicker_div = _getInputpickerDiv(input);
        var wrapped_list = _getWrappedList(input);
        var uuid = _uuid(input);
        // var data = _set(input, 'data');
        ...
        if(data.length){
            ...
            for(var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < fields.length; j++) {
                    ...
                    output_tds += '<div class="divCell pointer"';
                    var html_style = "";
                    if(_isObject(fields[j])){
                        if(fields[j]['width']){
                            html_style += "width:" + fields[j]['width'];
            ...
            output += "<div class='divTBody' style='column-span: all;'><div class='divRow'><div style='column-span: all;' class='divCell'>No results.</div></div></div>";
        }
        ...
        wrapped_list.find('div.divTBody').find('div.divRow').each(function () {
            ...
            that.on('mouseover', function (e) {
                ...
                _o(input).trigger('change', $(this).data());
